...
- The field is split into three zones:
  - Left paddle zone
  - Center Tetris zone (5 columns × 10 rows)
  - Right paddle zone
...
### Grid and 5 columns × 10 rows.
- Blocks are strictly square.
- Square size is derived from playable field height (`height / 10`), and zone width is `5
...
- Only one tetromino is active (falling) aon **each paddle hit** (not time-based).
